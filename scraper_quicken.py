#This file is used to scrape the data for the company Quicken from Trustpilot.
#The data is then stored in a csv file called 'data_quicken.csv', unaltered.
...
def main():
    ...
    for page in range(1, 1617):
        url = f'https://www.trustpilot.com/review/www.quicken.com?page={page}'
...
# Create a DataFrame from the collected data
    df_quicken = pd.DataFrame(all_reviews)
...
#Save csv in Github repo
    ...
    csv_filename = 'data_quicken.csv'
    full_path = os.path.join(data_dir, csv_filename)

# Save the DataFrame to a CSV file
    df_quicken.to_csv(full_path, index=False)
    print(f"Data saved to {csv_filename}")

# Output evaluation
    print("Total number of reviews:", len(df_quicken))
    print("Number of duplicate entries: ", df_quicken.duplicated().sum())

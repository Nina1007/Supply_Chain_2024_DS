#This file is used to scrape the data for the company Asda from Trustpilot.
#The data is then stored in a csv file called 'data_asda.csv', unaltered.
...
def scrape_page(url, page_number):
    ...
    for card in review_cards:
        try:
            ...
            answer_element = card.find('p', class_='typography_body-m__xgxZ_ typography_appearance-default__AAY17', 
                          attrs={'data-service-review-business-reply-text-typography': 'true'})
...
def main():
    ...
    for page in range(1, 797):
        url = f'https://www.trustpilot.com/review/www.asda.com?page={page}'
...
# Create a DataFrame from the collected data
    df_asda = pd.DataFrame(all_reviews)
...
#Save csv in Github repo
    ...
    csv_filename = 'data_asda.csv'
    full_path = os.path.join(data_dir, csv_filename)

# Save the DataFrame to a CSV file
    df_asda.to_csv(full_path, index=False)
    print(f"Data saved to {csv_filename}")

# Output evaluation
    print("Total number of reviews:", len(df_asda))
    print("Number of duplicate entries: ", df_asda.duplicated().sum())
